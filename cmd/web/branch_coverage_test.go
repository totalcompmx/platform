...
func TestHandlerBranchCoverage(t *testing.T) {
	...
	t.Run("API calculation errors", func(t *testing.T) {
		...
		app = apiTestApplication(t)
		app.writeAPIResponse(httptest.NewRecorder(), jsonRequestBody(t, `{"salary":100}`), map[string]any{"success": true})
		app.writeAPIValidationError(httptest.NewRecorder(), jsonRequestBody(t, `{"salary":100}`), []apiFieldError{{Field: "salary", Message: "bad"}})
...
func TestMiddlewareBranchCoverage(t *testing.T) {
	...
	t.Run("API auth branches", func(t *testing.T) {
		...
		app = newTestApplication(t)
		app.db.UpdateUserAPIKey(testUsers["alice"].id, hashAPIKey("api-key"), apiKeyPrefix("api-key"))
